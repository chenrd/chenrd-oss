...
public final class SingleSignOutHandler
{
    ...
    /**
     * 判断request对象是否正确（解决统一注销失败：多出一个回调/cas）
     * 
     * @param request
     * @return 
     * @see
     */
    public boolean isCorrectRequest(final HttpServletRequest request)
    {
        final String token = CommonUtils.safeGetParameter(request, this.artifactParameterName);
        final HttpSession session = request.getSession(true), sessionOld = ((HashMapBackedSessionMappingStorage) this.sessionMappingStorage).getSessionId(token);
        return sessionOld == null ? true : session.equals(sessionOld);
    }
