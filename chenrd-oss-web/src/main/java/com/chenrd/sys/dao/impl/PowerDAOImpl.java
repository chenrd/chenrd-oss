...
public class PowerDAOImpl extends AbstractBaseDAO<Power> implements PowerDAO {
	...
	public void addPower(Attribute power) {
		...
		Query query = super.getSession().createQuery("select count(*) from " + Power.class.getSimpleName()
				+ " as po where po.value=:value and po.parentKey = :parentKey");
		...
		query = super.getSession().createQuery(
				"select max(po.key) from " + Power.class.getSimpleName() + " as po where po.parentKey=:key");
	...
	public List<Power> findFilter(String applyKey) {
		StringBuilder hql = new StringBuilder("from ").append(Power.class.getSimpleName()).append(
				" as po where po.apply.key=:applyKey and po.status=:status and (po.type = :type1 or po.type = :type2) order by po.key");
