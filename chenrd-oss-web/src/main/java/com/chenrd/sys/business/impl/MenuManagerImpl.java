...
public class MenuManagerImpl extends AbstractPowerBusiness implements MenuManager {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.chenrd.sys.business.MenuManager#findChilds(java.lang.Long,
	 * java.lang.String)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.chenrd.sys.business.MenuManager#saveOrUpdate(com.chenrd.sys.vo.
	 * MenuVO)
	 */
	@Override
	@Transactional
	public void saveOrUpdate(MenuVO vo) {
		...
		menu.setApply(apply);
		menu.setKey((StringUtils.isNotBlank(vo.getParentKey()) ? vo.getParentKey() + "/"
				: apply.getKey() + "/" + PowerEnum.MENU.getKey() + "/") + vo.getKey());
		// 全称
		...
		if (vo.getFuncNames() != null) {
			for (int i = 0; i < vo.getFuncNames().length; i++) {
				...
				funcManager.saveOrUpdate(new PowerInfo(
						vo.getFuncIds() == null || (vo.getFuncIds().length - 1) < i ? null : vo.getFuncIds()[i],
						vo.getFuncNames()[i], (i + 1) * 100 + "", vo.getFuncUrls()[i], menu.getKey(), apply.getId(),
						apply.getKey()));
			}
		}
	}
	...
	public MenuVO get(Long id) {
		...
		vo.setKey(menu.getApply().getKey().equals(menu.getParentKey())
				? menu.getKey().replace(menu.getApply().getKey() + "/" + PowerEnum.MENU.getKey() + "/", "")
				: menu.getKey().replace(menu.getParentKey() + "/", ""));
		return vo;
	}
