...
public class UserManagerImpl extends AbstractPowerBusiness implements UserManager {
	...
	private UserVO copyEntity(User user) {
		...
		if (user.getPowers() != null) {
			for (Power power : user.getPowers()) {
				tmp.append(power.getId()).append(",");
			}
		}
		info.setPowerstrs(tmp.toString());
		
		tmp.delete(0, tmp.length());
		if (user.getRoles() != null) {
			for (Role role : user.getRoles()) {
				tmp.append(role.getId()).append(",");
			}
		}
		info.setRolestrs(tmp.toString());

		tmp.delete(0, tmp.length());
		if (user.getApplys() != null) {
			for (Apply apply : user.getApplys()) {
				tmp.append(apply.getId()).append(",");
			}
		}
		info.setApplystrs(tmp.toString());
		return info;
	}

	@Override
	public void allotField(Long fieldId, String... userIds) {
	...
	@Override
	public void deleteField(Long fieldId, String... userIds) {
		for (String userId : userIds) {
